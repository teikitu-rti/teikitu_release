...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (MAC) Kernel - Internal.h
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= */
#if !defined(TGS_MAC_KERNEL_INTERNAL_H)
#define TGS_MAC_KERNEL_INTERNAL_H
...
#include "TgS (MAC) Kernel - Internal - Type.h"
#include "TgS (MAC) Kernel - Internal - Data.h"

#include "TgS COMMON/TgS Common.inl"

#if !defined(TgBUILD_OS__MAC)
#error This file should only be included for MAC based platforms
#endif
