...
TextureCube g_sampler_cube[KTgMACRO_KN_GPU_EXT_MAX_DEFAULT_CUBE] : register(t1000);
Texture3D g_sampler_volume[KTgMACRO_KN_GPU_EXT_MAX_DEFAULT_VOLUME] : register(t1004);
Texture2D g_sampler_image[KTgMACRO_KN_GPU_EXT_MAX_DEFAULT_IMAGE] : register(t1008);
...
ConstantBuffer<STg2_KN_GPU_DBG_Geom_Instance> DBG_Geom_Instance : register(b1000);
ConstantBuffer<STg2_KN_GPU_DBG_Geom> Debug_Model : register(b1000);
ConstantBuffer<STg2_KN_GPU_DBG_Text> Debug_Text : register(b1000);
ConstantBuffer<STg2_KN_GPU_DBG_Quad> Debug_UI : register(b1000);
ConstantBuffer<STg2_KN_GPU_DBG_UBO_Line> Debug_Line : register(b1000);
...
ConstantBuffer<STg2_KN_GPU_Unit_Test_Scene_UBO> Unit_Test_Scene : register(b1001);
...
float3 rgb2hsl(float3 colour){
    float4 K = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
    float4 p = lerp(float4(colour.bg, K.wz), float4(colour.gb, K.xy), step(colour.b, colour.g));
    float4 q = lerp(float4(p.xyw, colour.r), float4(colour.r, p.yzx), step(p.x, colour.r));
...
float3 hsl2rgb(float3 colour)
{
    float4 K = float4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
    float3 p = abs(frac(colour.xxx + K.xyz) * 6.0 - K.www);
    return colour.z * lerp(K.xxx, clamp(p - K.xxx, 0.0, 1.0), colour.y);
}


float3 HDRCorrection( in float3 colour_709, in STg2_KN_GPU_Output_DESC sOutputDesc )
{
    float3 result = colour_709;

    if (sOutputDesc.m_bHDR_Enabled)
    {
        if (KTgMACRO_KN_GPU_HLSL_COLOUR_SPACE_G2084_NONE_P2020 == sOutputDesc.m_iColour_Space)
        {
        ...
        else if (KTgMACRO_KN_GPU_HLSL_COLOUR_SPACE_G10_NONE_P709 == sOutputDesc.m_iColour_Space)
        {
    ...
    {
        if (KTgMACRO_KN_GPU_HLSL_COLOUR_SPACE_G10_NONE_P709 != sOutputDesc.m_iColour_Space)
        {
