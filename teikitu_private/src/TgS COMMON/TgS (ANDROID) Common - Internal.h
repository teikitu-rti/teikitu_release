...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (ANDROID) Common - Internal.h
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#if !defined(TGS_ANDROID_COMMON_INTERNAL_H)
#define TGS_ANDROID_COMMON_INTERNAL_H
...
#include TgHEADER_THREAD_SUPPORT(TgS,Common - Internal [Thread].h)
