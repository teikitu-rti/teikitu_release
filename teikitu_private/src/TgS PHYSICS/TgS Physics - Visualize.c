...
typedef struct {
    ...
    STg2_KN_GPU_DBG_Geom_P                      m_psGeom;
...
tgPH_Form_Visualize_Render_Internal(
    STg2_KN_GPU_CMD_PC ARG0, STg2_PH_Visualize_Form ARG1[ETgKN_GPU_DEBUG_PM_3D_COUNT], TgPH_WORLD_ID_C ARG2, TgBOOL (*ARG3)( TgVEC_UN_F32_04_1_PC, STg2_PH_Form_CPC, STg2_PH_Body_CPC ));
...
static TgFLOAT64 s_afSmooth__Build_Sets_Inner_Total[TgMACRO_MAX_SMOOTH_FRAMES];
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID tgPH_Debug_Render_Performance_Stats( STg2_KN_GPU_CMD_PC psCMD, STg2_KN_GPU_Render_Buffer_CPC psRTBuffer )
{
    STg2_KN_GPU_FONT_TX                 sFont_TX;
    STg2_KN_GPU_DBG_Text_CI             sText;
    ...
    tgMM_Set_U08_0x00( &sText, sizeof( sText ) );
    puszText = uszBuffer;

    sText.m_enFont = ETgKN_GPU_DOS_FONT_ROM_MARCIO;
    sText.m_puszText = &puszText;
    sText.m_nuiText = 1;
    sText.m_vText_Box_V.x = -1.0F + (10.0F / (float)psRTBuffer->m_uiWidth);
    sText.m_vText_Box_V.y = -1.0F + (10.0F / (float)psRTBuffer->m_uiHeight);
    sText.m_vText_Box_V.z = sText.m_vText_Box_V.x + ((4.0F + sFont_TX.m_fFntX * 34 * 2) / (float)psRTBuffer->m_uiWidth);
    sText.m_vText_Box_V.w = sText.m_vText_Box_V.y + ((sFont_TX.m_fFntY * 10 * 2) / (float)psRTBuffer->m_uiHeight);
    sText.m_vText_Colour.r = 0.0F;
    sText.m_vText_Colour.g = 1.0F;
    sText.m_vText_Colour.b = 0.0F;
    sText.m_vText_Colour.a = 1.0F;
    sText.m_bRight_Aligned = false;
    sText.m_bDraw_Background = true;
    sText.m_vBackground_Colour.r = 0.0F;
    sText.m_vBackground_Colour.g = 0.0F;
    sText.m_vBackground_Colour.b = 0.0F;
    sText.m_vBackground_Colour.a = 0.15F;
    ...
    s_afSmooth__Build_Sets_Inner_Total[s_nuiStat_Call % TgMACRO_MAX_SMOOTH_FRAMES] = PROFILE_TOTAL_TIME_CACHED( PH_BUILD_SETS_INNER_TOTAL );
    ...
    tgKN_GPU__CMD__Render_Debug_Text_Box( psCMD, &sText );

    sText.m_bDraw_Background = false;
    ...
    for (afSmooth_Total = 0.0, uiIndex = 0; uiIndex < TgMACRO_MAX_SMOOTH_FRAMES; ++uiIndex) { afSmooth_Total += s_afSmooth__Collision_Contact_Callback_Total[uiIndex] / (TgFLOAT64)TgMACRO_MAX_SMOOTH_FRAMES; }         tgUSZ_AppendF( uszBuffer, 256, u8"     -Count: %d\n", TgSTD_ATOMIC(load_explicit)( &g_xnuiFM_Callback, TgSTD_MEMORY_ORDER(relaxed) ) );
/*# defined(TgS_STAT__PHYSICS) && TgS_STAT__PHYSICS */
    ...
    for (afSmooth_Total = 0.0, uiIndex = 0; uiIndex < TgMACRO_MAX_SMOOTH_FRAMES; ++uiIndex) { afSmooth_Total += s_afSmooth__Build_Sets_Inner_Total[uiIndex] / (TgFLOAT64)TgMACRO_MAX_SMOOTH_FRAMES; }                   tgUSZ_AppendF( uszBuffer, 256, u8"+-Build:                  %8.2f\n", afSmooth_Total );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID tgPH_Form_Visualize_Render( STg2_KN_GPU_CMD_PC psCMD, STg2_KN_GPU_DBG_Geom_PC psModelConstantBuffer, TgPH_WORLD_ID_C tiWorld, ETgKN_GPU_EXT_FORMAT_C enRT,
                                   ETgKN_GPU_EXT_FORMAT_C enDS )
{
    ...
    for (uiIndex = 0; uiIndex < TgARRAY_COUNT(asData); ++uiIndex)
    {
        ...
        tgMM_Copy( &asData[uiIndex].m_sInstance, sizeof(STg2_KN_GPU_DBG_Geom_Instance), psModelConstantBuffer, sizeof(STg2_KN_GPU_DBG_Geom) );
    };
    ...
    tgKN_GPU_EXT__CMD__Set_State_From_Default( psCMD, ETgKN_GPU_DBG_GRAPHICS_PIPELINE_GEOM_02_INSTANCE, enRT, enDS );
    ...
    tgPH_Form_Visualize_Render_Internal( psCMD, asData, tiWorld, tgPH_Form_Visualize_Render__Iteration_Loop_Kinematic );
    tgPH_Form_Visualize_Render_Internal( psCMD, asData, tiWorld, tgPH_Form_Visualize_Render__Iteration_Loop_Dynamic );
    ...
    {
        tgMH_Init_T_F32_04_4( asData[iSelect].m_sInstance.m_avModel_Transform + asData[iSelect].m_nuiInstance, g_auPH_Debug__Contact[uiIndex].m_vS_F32_04_1 );
        ...
        asData[iSelect].m_sInstance.m_vModel_Colour[asData[iSelect].m_nuiInstance] = uColour.m_vS_F32_04_1;

        ++asData[iSelect].m_nuiInstance;
        if (asData[iSelect].m_nuiInstance >= TgARRAY_COUNT(asData[iSelect].m_sInstance.m_avModel_Transform))
        {
            tgKN_GPU_EXT__CMD__Render_Debug_Geom_Instance( psCMD, (ETgKN_GPU_DEBUG_PM)((TgSINT_E32)ETgKN_GPU_DEBUG_PM_3D_START + iSelect), asData[iSelect].m_nuiInstance,
...
#if defined(TgS_DEBUG__PHYSICS) && TgS_DEBUG__PHYSICS && defined(TgCOMPILE__RENDER_DEBUG_OUTPUT)
static TgVOID tgPH_Form_Visualize_Render_Internal( STg2_KN_GPU_CMD_PC psCMD, STg2_PH_Visualize_Form asData[ETgKN_GPU_DEBUG_PM_3D_COUNT], TgPH_WORLD_ID_C tiWorld,
                                                   TgBOOL (*ARG0)( TgVEC_UN_F32_04_1_PC, STg2_PH_Form_CPC, STg2_PH_Body_CPC ))
{
    ...
    for (uiIndex = 0; uiIndex < nuiForm_World; ++uiIndex)
    {
        ...
        case ETgPM_PN:
            ...
            tgMM_Copy( asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform + asData[iSelect].m_nuiInstance, sizeof(TgVEC_S_F32_04_4), uM0.m_pmS_F32_04_3, sizeof(TgVEC_UN_F32_04_3) );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC3 = KTgUNIT_W_S_F32_04_1;
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_vModel_Colour = uColour.m_vF32_04_1;
        ...
        case ETgPM_BX: {
            ...
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC0 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM3.m_mS_F32_04_3.m_vC0 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC1 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM3.m_mS_F32_04_3.m_vC1 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC2 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM3.m_mS_F32_04_3.m_vC2 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC3 = KTgUNIT_W_S_F32_04_1;
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_vModel_Colour = uColour.m_vF32_04_1;
        ...
        case ETgPM_SP: {
            ...
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC0 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM0.m_pmS_F32_04_3->m_vC0 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC1 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM0.m_pmS_F32_04_3->m_vC1 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC2 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM0.m_pmS_F32_04_3->m_vC2 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC3 = KTgUNIT_W_S_F32_04_1;
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_vModel_Colour = uColour.m_vF32_04_1;
        ...
        case ETgPM_CP: {
            ...
            tgMH_CAT_F32_04_3( &uM3.m_mF32_04_3, &psFM->m_uPM.m_sTB.m_mReference_Frame, uM0.m_pmF32_04_3 );
            ...
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC0 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM5.m_mS_F32_04_3.m_vC0 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC1 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM5.m_mS_F32_04_3.m_vC1 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC2 = tgMH_MUL_S_F32_04_1( uScale.m_vS_F32_04_1, uM5.m_mS_F32_04_3.m_vC2 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform.m_vC3 = KTgUNIT_W_S_F32_04_1;
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_vModel_Colour = uColour.m_vF32_04_1;
        ...
        for (TgRSIZE uiVert = 0; uiVert < 8; ++uiVert)
        {
            ...
            tgMH_Init_T_F32_04_4( asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform, uVert.m_vS_F32_04_1 );
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform._11 *= 0.05F;
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform._22 *= 0.05F;
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_avModel_Transform._33 *= 0.05F;
            asData[iSelect].m_sGeom[asData[iSelect].m_nuiInstance].m_vModel_Colour = uColour.m_vF32_04_1;
    ...
    for (uiIndex = 0; uiIndex < ETgKN_GPU_DEBUG_PM_3D_COUNT; ++uiIndex)
    {
        ...
        tgKN_GPU_EXT__CMD__Render_Debug_Geom_Instance( psCMD, (ETgKN_GPU_DEBUG_PM)((TgSINT_E32)ETgKN_GPU_DEBUG_PM_3D_START + uiIndex), asData[uiIndex].m_nuiInstance,
