...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (REF) Kernel [GPU] - Constants.h
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#if !defined(TGS_REF_KERNEL_GPU_CONSTANTS_H) && defined(TgBUILD_FEATURE__GRAPHICS)
#define TGS_REF_KERNEL_GPU_CONSTANTS_H
...
typedef enum TgATTRIBUTE_ENUM
{
    ETgKN_GPU_EXT_TOPOLOGY_POINT_LIST,
    ETgKN_GPU_EXT_TOPOLOGY_LINE_LIST,
    ETgKN_GPU_EXT_TOPOLOGY_TRIANGLE_LIST,
    ETgKN_GPU_EXT_TOPOLOGY_PATCH_LIST,
    ETgKN_GPU_EXT_TOPOLOGY_TRIANGLE_STRIP,
