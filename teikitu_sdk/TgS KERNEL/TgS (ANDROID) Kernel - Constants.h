...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (ANDROID) Kernel - Constants.h
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= */
#if !defined(TGS_ANDROID_KERNEL_CONSTANTS_H)
#define TGS_ANDROID_KERNEL_CONSTANTS_H
...
enum { KTgKN_MAX_WINDOWS                    =     1 };  /**< Maximum number windows */
